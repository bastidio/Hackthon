...
def agregar_faker_usuario(cantidad=5):
    ...
    for i in range(cantidad):
        ...
        clave = str(random.randint(100000, 999999))

        while usuarioExistente(usuario):
            usuario = "usuario" + str(random.randint(100, 999))

        arch.write(f"{usuario},{nombre},{dni},{clave}\n")
        print(f"Usuario Faker agregado: {usuario} | Nombre: {nombre} | DNI: {dni}")
...
def main():
    ...
    while True:
        ...
        if op == "1":
            ...
            dicc = {}
            dicc = generaDiccionario(dicc)
            generaReportePorcentaje(dicc)
